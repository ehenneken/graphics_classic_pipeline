...
from models import db, GraphicsModel, AlchemyEncoder
...
class BackupDatabase(Command):
    option_list = (
        Option('--set', '-s', dest='set'),
    )

    def run(self, **kwargs):
        """
        Checks graphics database for a specific record
        """
        backupdirbase = '/proj/ads/articles/graphics/backup'
        if kwargs.get('set'):
            source = kwargs.get('set')
            backupdir = "{0}/{1}".format(backupdirbase, source)
            if not os.path.exists(backupdir):
                os.mkdir(backupdir)
            if not os.path.exists(backupdir):
                sys.exit('Backup directory does not exist: {0}'.format(backupdir))
            with create_app().app_context():
                query = db.session.query(GraphicsModel).filter(
                         GraphicsModel.source == source)
                for rec in query.all():
                    ofile = "{0}/{1}.json".format(backupdir, rec.bibcode)
                    if os.path.exists(ofile):
                        continue
                    with open(ofile, 'w') as f:
                        json.dump(rec, f, cls=AlchemyEncoder, indent=4)
#                    results = json.loads(json.dumps(rec, cls=AlchemyEncoder))
#                    print results
        else:
            sys.exit('No valid request supplied!\n')

class CheckDatabaseEntry(Command):
    option_list = (
    ...
    def run(self, **kwargs):
        ...
        if kwargs.get('identifier'):
            bibcode = kwargs.get('identifier')
            with create_app().app_context():
                record = db.session.query(GraphicsModel).filter(
						            GraphicsModel.bibcode == bibcode).first()
                if not record:
                   sys.exit('No record found for {0}\n'.format(bibcode))
        ...
        if kwargs.get('identifier'):
            ...
            with create_app().app_context():
                ...
                for f in record.figures:
                    try:
#                        thumbnails.append(f['images'][0]['thumbnail'])
            ...
            with create_app().app_context():
                ...
                for record in query.all():
                    ...
                    try:
                        ...
                        nthmbs = 0
                    if nthmbs > 0:
                        sys.stderr.write('Already processed. Skipping {0}\n'.format(bibcode))
                        continue
                    print "processing: {0}".format(bibcode)
            ...
            for identifier in kwargs.get('identifier').split(','):
                ...
                for pset in set2journal:
                    if [j for j in set2journal[pset] if j in identifier]:
                        if pset == 'EDP' and ident['bibcode'][4:9] == 'ARA&A':
                            continue
...
manager.add_command('checkdb', CheckDatabaseEntry())
manager.add_command('migratedb', MigrateDatabase())
manager.add_command('backupdb', BackupDatabase())
